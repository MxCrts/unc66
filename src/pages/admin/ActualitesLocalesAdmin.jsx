...
export default function ActualitesLocalesAdmin() {
  ...
  async function loadItems() {
    ...
    setLoadError("");
    try {
      setItems(await listActualitesLocales());
    } catch (err) {
  ...
  function handleEdit(item) {
    ...
    setForm({
      ville: item.ville || VILLES_ASSOCIATIONS[0],
      titre: item.titre || "",
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-unc-navy mb-2">Actualités des associations locales</h1>
      <p className="text-sm text-unc-gray/70 mb-6">
        Publiez une actualité au nom d'une commune. Elle apparaît dans la section « Actualités des
        associations locales » de la page publique, sous la commune choisie.
      </p>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-unc-border/30 rounded-lg p-5 space-y-4 mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold uppercase tracking-wide text-unc-navy">
            {editingId ? "Modifier l'actualité locale" : "Nouvelle actualité locale"}
          </h2>
          {editingId && (
      ...
      <h2 className="text-sm font-bold uppercase tracking-wide text-unc-navy mb-3">
        Actualités locales existantes ({items.length})
      </h2>
      ...
      {loading ? (
        ...
        <p className="text-sm text-unc-gray/60">Aucune actualité locale pour l'instant.</p>
      ) : (
